...
export default function ProductCard({ product }) {
  ...
  const firstImage = Array.isArray(image_id) ? image_id[0] : (image_id || null);
  const imageUrl = firstImage?.url || (firstImage?.path ? buildProductImageUrl(firstImage.path) : (image || "/Images/best-1.svg"));

  const displayNotes = notes || description || "";
  const displayCategory = category_id?.name || category?.name || category || tag_id?.name || "Unisex";
